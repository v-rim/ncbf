...
from ncbf.env import Environment
from ncbf.filter import NullFilter, NeuralFilter, QPFilter
from ncbf.nn import MLP
from ncbf.policy import Policy
from ncbf.renderer import NotebookRenderer
...
def collect_ncbf(args):
    ...
    """
    policy = Policy(args, NullFilter(args))
...
def train_ncbf(args, rollout_data):
    ...
    for epoch in (pbar := tqdm(range(epochs))):
        ...
        pbar.set_description(f"[{epoch+1}/{epochs}] | Loss: {loss.item():.4f}")
...
def collect_qp(args, ncbf_model):
    ...
    # Each row is the current state + the optimal QP solution (a vector in R^2)
    rollout_data = np.zeros(
        (
            args["rollouts"],
            args["steps_per_rollout"],
...
def test_qp(args, qp_model):
    ...
    for i in range(args["rollouts"]):
        # Prepare for rollout
        env.reset([0, 60])  # Start at positions that likely require a QP solution
